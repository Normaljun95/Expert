...
class Consumer(threading.Thread):
    ...
    def search_Name(self, name, tempAids, idx):
        search_name = self.dbs['Author'].find_one({"_id" : tempAids[idx]})
        return search_name['name'] if search_name is not None else name
    ...
    def insertID(self, name, inst, tempAids, idx, site):
        ...
        if len(tempAids) == 0 :
            tempId = "ss"+str(self.cnt)  # 중복이 없으면 아이디 부여
            self.cnt +=1
        else :
            tempId = tempAids[idx]
            if self.site == 'DBPIA':
    ...
    """ Note #6 저자 ID 생성 helper 6 (2022.01.06. 추가) """
    """
        @isEnglishOrKorean    : 저자 ID 검색
            @input_s             : 대학 소속
    """
    #@logging_time
    def isEnglishOrKorean(self, input_s):
        ...
        try:
            ...
            print(f'예외문자: {input_s}')
            return "e"

    """ Note #7 저자 ID 생성 helper 7 (2022.01.06. 추가) """
    """
        @check_college    : 대학 DB 검색
        @univ0            : 대학 소속
    """
    #@logging_time
    ...
    def getAIds(self, data):
        ...
        if self.site == 'NTIS' :
            ...
            if data['perfAgent'] is not None and data['perfAgent']['@code'] == '03':
                ...
                (data["originalName"], instbool) = self.check_college(Inst) # Update 2022.01.07
...
# WOS r_id, o_id 삽입 및 업데이트
            ...
            if not author_inst: # 저자의 소속이 없을때(SCIENCEON), DBPIA
                for name in names:
                    if self.site == 'SCOPUS':
                        processedNames += self.search_Name(name, tempAids, idx)+";"
                    else:
                        processedNames += name+";"
                ...
                for Name,inst in zip(names, author_inst):  # 이름, 소속 둘다 있으면
                    ...
                    #InstList = processedInsts.split(';') # Update 2022.01.07
                    #for Ins in range(len(InstList)-1):
                    (Ins, instBool) = self.check_college(inst)
                    collegeInsts += Ins+";"
            ...
            if len(processedInsts) != 0 :
                data['author_inst'] = processedInsts
                data['originalName'] = collegeInsts # Update 2022.01.07
