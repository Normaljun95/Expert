...
class factor_integration(threading.Thread):
    ...
    def run(self):
        ...
        for i in range(allPage):
            ...
            # accuracy = self.acc(keywords, contBit, querykey, self.stop_words)
            accuracy = self.acc(keywords, querykey) #, self.stop_words, self.komoran)
            ...
            for doc1 in base_data:
                ...
                doc1['ngvCoop']       = self.NGV_Coop(doc1)
                doc1['LDA']           = lda_ls[count_base_data]
        ...
        for i in range(len(getBackdata) - 1, -1, -1):
            ...
            if (getBackdata[i]['ntis'] != None):
                ...
                for doc in self.ntis_client['Rawdata'].find({"keyId": keyID, "_id": {"$in" : getBackdata[i]['ntis papers']}}):
                    ...
                    # added to Code
                    __keyword.append(doc['goalAbs'])
                    __keyword.append(doc['absAbs'])
                    __keyword.append(doc['effAbs'])
    ...
    def acc(self, keywords, querykey): #, stop_words, komoran):
        rtv = [0 for r in range(len(keywords))]
        for i in range(len(keywords)):
            # temp = calAcc(keywords[i], querykey)
            temp = calAcc(keywords[i], querykey, self.stop_words) # , self.komoran)
            # print(temp)
            if temp == 0.0 :
                rtv[i] = 0.02 
            else :
    ...
    def NGV_Coop(self, doc1):
        for coop_cursor in self.ID['coopList'].find({'name':doc1['name'], 'college':doc1['inst']}):
            #print(coop_cursor)
            ngvCoop_score = coop_cursor['ngvCoop']
            return ngvCoop_score


def calAcc(keywords, querykey, stop_words): #, komoran):
    # print("calAcc")
    # komoran = Komoran()
    flat_list = []
...
def LDA_model(keywords, stop_words):
    ...
    for sublist in keywords:
        ...
        for item in sublist:
            if item is not None and item != 'None' and item != "" and isinstance(item, str) :
                    #flat_list.append([word for word, pos in nltk.pos_tag(nltk.word_tokenize(item)) if (pos == 'NN' or pos == 'NNP' or pos == 'NNS' or pos == 'NNPS')])
