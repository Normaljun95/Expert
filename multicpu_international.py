...
from new_analyzer_4 import factor_integration
...
class run_factor_integration:
    def __init__(self, keyid, fid):
        ...
        self.International = self.ID['International']
        self.keyid = keyid
        self.fid = fid
    ...
    def run(self):
        print("multicpu run")
        authorSize = self.ID['International'].find({"keyId":self.keyid, "fid":self.fid}).count()
        processList = []
        ...
        if None == self.new_max_factor.find_one({'keyId': self.keyid,"fid":self.fid}):
            ...
            self.new_max_factor.insert({'keyId': self.keyid,"fid":self.fid,'keyId': self.keyid,"fid":self.fid, 'interQual' : -1,'accuracy' : -1, 'recentness' : -1, 'coop': -1 })
    ...
    def factor_norm(self):
        ...
        max_interQual = max_factor['interQual']
        
        norm_interQual_factor = 1 / max_interQual
        ...
        self.ID['test'].update_many(
            {"keyId":self.keyid, "fid":self.fid}, [
                {"$set" : {
                    ...
                    "factor.qual":{
                            "$sum":[
                                {"$multiply" : ["$factor.interQual", norm_interQual_factor,0.5]}
                            ]},
                    "score" : {
                        '$sum':[
