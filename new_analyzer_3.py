...
class factor_integration(threading.Thread):
    ...
    def __init__(self, start, num_data, fid, keyID, stop_words):
        ...
        if self.fid == 0:
            ...
            for doc in self.FilterCategory.find({'keyId' : self.keyId, 'fId' : self.fid}):
                ...
                project_inst    = doc['project']['inst']['list']

            self.base_insts     = defaultdict(list)
            for k, v in chain(paper_inst.items(), project_inst.items()):
    ...
    def coop(self, _coopBackdata):
        ...
        @ _coopBackdata   : getRawBackdata 함수에서 mngIds, A_ID 값을 가지고 있는 변수
        """
...
def LDA_model(keywords, stop_words):
    """
    @ Method Name     : LDA_model
    @ Method explain  : LDA 모델을 이용한 잠재 키워드 추출 함수(실제 lda 함수에서 각 논문/프로젝트 잠재 키워드 추출)
    @ keywords        : 1) 논문 제목, 프로젝트 키워드 (in project) 
                        2) 논문 제목, 논문 키워드, abstract (in paper)
    @ stop_words      : 불용어
    """
